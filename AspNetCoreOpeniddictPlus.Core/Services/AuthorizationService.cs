...
public class AuthorizationService<IEntity>(
    ...
    UserManager<IEntity> userManager
) : Controller, IAuthorizationService
    where IEntity : class
{
    public virtual async Task<IActionResult> AuthorizeAsync()
    {
        ...
        if (request.HasPromptValue(OpenIddictConstants.PromptValues.Login))
        {
            ...
            var prompt = string.Join(
                " ",
                request.GetPromptValues().Remove(OpenIddictConstants.PromptValues.Login)
            );

            var parameters = Request.HasFormContentType
                ? Request
                    .Form.Where(parameter => parameter.Key != OpenIddictConstants.Parameters.Prompt)
                    .ToList()
                : Request
                    .Query.Where(parameter =>
                        parameter.Key != OpenIddictConstants.Parameters.Prompt
                    )
                    .ToList();

            parameters.Add(
                KeyValuePair.Create(OpenIddictConstants.Parameters.Prompt, new StringValues(prompt))
            );
        ...
        await foreach (
            var authorization in authorizationManager.FindAsync(
        ...
        switch (await applicationManager.GetConsentTypeAsync(application))
        {
            ...
            default:
                return View(
                    new AuthorizationServiceResult
                    {
                        ApplicationName = await applicationManager.GetLocalizedDisplayNameAsync(
                            application
                        ),
                        Scope = request.Scope,
                    }
    ...
    public virtual async Task<IActionResult> AcceptAsync()
    {
        ...
        if (
            ...
            && await applicationManager.HasConsentTypeAsync(
                application,
    ...
    private async Task<IActionResult> HandleExchangeClientCredentialsGrantType(
